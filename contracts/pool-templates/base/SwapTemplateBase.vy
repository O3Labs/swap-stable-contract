...
def _add_liquidity(_provider: address, _amounts: uint256[N_COINS], _min_mint_amount: uint256) -> uint256:
    ...
    for i in range(N_COINS):
        if _amounts[i] > 0:
            ...
            if len(_response) > 0:
                assert convert(_response, bool), "ERR_ERC20_FALSE"
...
def ramp_A(_future_A: uint256, _future_time: uint256):
    ...
    assert _future_time > self.initial_A_time
...
@external
def set_transfer_ownership(_owner: address):
    assert msg.sender == self.owner
